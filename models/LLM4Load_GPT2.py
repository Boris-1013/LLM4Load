...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        if configs.llm_model == 'GPT2':
            self.gpt_config = GPT2Config.from_pretrained('/modelscope/hub/iiBcai/gpt2/')
            #self.llama_config = LlamaConfig.from_pretrained('huggyllama/llama-7b')
            self.gpt_config.num_hidden_layers = configs.llm_layers
            self.gpt_config.output_attentions = True
            self.gpt_config.output_hidden_states = True
            self.llm_model = GPT2Model.from_pretrained(
                "/modelscope/hub/iiBcai/gpt2/",
                trust_remote_code=True,
                local_files_only=True,
            )
            
            self.tokenizer = GPT2Tokenizer.from_pretrained( "/modelscope/hub/iiBcai/gpt2/",
                trust_remote_code=True,
                local_files_only=True,
            )
